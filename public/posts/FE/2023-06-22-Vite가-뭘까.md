...
당연히 개발 서버로 실행하는 건 `dev`일 거고, 빌드 파일을 뽑아내거나 배포할 때 사용하는 것은 `build`일 것 같네요. 근데 빌드 시에 tsc를 사용하네요? 왠지 예전 [제 글](/post/FE/2022-10-15-ts-loader-vs-babel-loader-vs-esbuild-loader.md)이 생각나네요 후후 😎
