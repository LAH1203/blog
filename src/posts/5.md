...
```bash
yarn add -D @types/node @types/react @types/react-dom fork-ts-checker-webpack-plugin html-webpack-plugin typescript webpack webpack-cli webpack-dev-server

// or

npm install -D @types/node @types/react @types/react-dom fork-ts-checker-webpack-plugin html-webpack-plugin typescript webpack webpack-cli webpack-dev-server
```
...
그와 달리 `babel-loader`는 빌드 시 타입을 제거해요. `@babel/preset-typescript`이라는 플러그인을 사용해야지만 타입 체킹이 가능하죠. 또한 빌드 속도도 빠른 편입니다.
