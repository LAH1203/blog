...
개인의 취향에 따라 설정하면 되지만 보통 `ts-loader`를 많이 쓰는 듯 해요. 만약 개발 모드와 배포 모드 두 가지 버전으로 웹팩 설정을 나눈다면 저 같으면 개발 모드에서는 타입 체크를 엄격하게 하는 `ts-loader`, 배포 모드에서는 빌드 속도가 빠른 `babel-loader`를 사용할 것 같기는 해요. (최근에는 [esbuild-loader](https://github.com/privatenumber/esbuild-loader)라는 새로운 로더도 등장했대요! 빌드 속도가 babel-loader보다도 빠르다네요.)
